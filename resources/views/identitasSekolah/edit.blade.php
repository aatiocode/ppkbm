	...
	<div class="card card-unit-risk mt-4">
		<div class="card-body">
			<nav class="navbar bg-success" style="border-radius: 2px;">
				<h4 class="text-white mb-0">{{$artikel->view_only ? "Lihat Identitas Sekolah" : "Edit Identitas Sekolah"}}</h4>
			</nav>
			<form class="col-lg-12 col-md-12 mt-4" action="{{url("/admin/identitas-sekolah/".base64_encode($artikel->id)."/edit")}}" method="post" enctype="multipart/form-data">
				@method('put')
				@csrf
        ...
        @endif
				<a href="{{url('/admin/identitas-sekolah')}}" class="btn btn-danger" role="button" aria-pressed="true">Kembali</a>
			</form>
		</div>
	</div>
@endsection