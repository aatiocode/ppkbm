	...
	<div class="card card-unit-risk mt-4">
		<div class="card-body">
			<nav class="navbar bg-success" style="border-radius: 2px;">
				<h4 class="text-white mb-0">{{$galeri->view_only ? "Lihat Gambar" : "Edit Gambar"}}</h4>
			</nav>
			<form class="col-lg-12 col-md-12 mt-4" action="{{url('/admin/carousel/'.base64_encode($galeri->id).'/edit')}}" method="post" enctype="multipart/form-data">
				@method('put')
				@csrf
        ...
        <label for="id_event" class="hide">Kategori</label>
        ...
        @endif
				<a href="{{url('/admin/carousel/')}}" class="btn btn-danger" role="button" aria-pressed="true">Kembali</a>
			</form>
		</div>
	</div>
@endsection