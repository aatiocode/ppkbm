	...
	<div class="card card-unit-risk mt-4">
		<div class="card-body">
			<nav class="navbar bg-success" style="border-radius: 2px;">
				<h4 class="text-white mb-0">{{$artikel->view_only ? "Lihat Testimoni" : "Edit Testimoni"}}</h4>
			</nav>
			<form class="col-lg-12 col-md-12 mt-4" action="{{url("/admin/testimoni/".base64_encode($artikel->id)."/edit")}}" method="post" enctype="multipart/form-data">
				@method('put')
				@csrf
        ...
        <div class="form-group">
					...
					<input value="{{$artikel->nama}}" type="text" class="form-control" id="nama" name="nama" maxlength="200" {{$artikel->view_only ? "disabled" : ""}} required>
				</div>
        <div class="form-group">
					<label for="kota">Kota</label>
					<input value="{{$artikel->kota}}" type="text" class="form-control" id="kota" name="kota" maxlength="200" {{$artikel->view_only ? "disabled" : ""}} required>
				</div>
        <div class="form-group">
					<label for="deskripsi_singkat">Deskripsi Singkat</label>
					<input value="{{$artikel->deskripsi_singkat}}" type="text" class="form-control" id="deskripsi_singkat" name="deskripsi_singkat" maxlength="200" {{$artikel->view_only ? "disabled" : ""}} required>
				</div>
        <div class="form-group">
        ...
        @endif
				<a href="{{url('/admin/testimoni')}}" class="btn btn-danger" role="button" aria-pressed="true">Kembali</a>
			</form>
		</div>
	</div>
@endsection